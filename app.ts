...
// ----------------------------------------------------------------------------
// 1. Domain Types & Type Guards
// ----------------------------------------------------------------------------

type FactionName = "NATO" | "CSAT" | "AAF" | "FIA";
type RoleName = "Rifleman" | "Medic" | "Marksman" | "Anti-Tank" | "Machine Gunner" | "Sniper" | "Pilot" | "Pointman";
type ModSource = "Vanilla" | "RHS" | "CUP" | "NIArms" | "Custom";
type Caliber = "5.56x45" | "5.45x39" | "7.62x39" | "7.62x51" | "7.62x54" | "6.5x39" | "5.8x42" | "9x21" | ".45ACP" | "4.6x30" | ".300WM" | ".338" | "9.3x64" | ".408" | "12.7x108" | ".50BMG" | "12Gauge" | "Rocket";
type OpticProfile = "cqb" | "mid" | "long";
type TierLevel = "standard" | "specops" | "militia";
...
interface WeaponDef {
    ...
    tier?: TierLevel;
    caliber: Caliber;
    defaultMag: [string, number];
    opticType: OpticProfile;
    hasBipod?: boolean;
    defaultBipod?: string;
    hasMuzzle?: boolean;
    photoUrl: string;
...
interface FactionGear {
    uniforms: string[];
    vests: string[];
    backpacks: string[];
    headgear: string[];
    handguns: string[];
    handgunMag: [string, number];
    handgunMuzzles?: string[];
}
...
interface LoadoutMeta {
    chaosLevel: number;
    primaryMod: ModSource;
    caliber: Caliber;
}
...
interface GeneratorOptions {
    faction: FactionName | "Random";
    role: RoleName | "Random";
    chaosLevel: number; // 1: Authentic, 2: SpecOps / PMC, 3: Cursed / Chaos
    activeMods: Set<ModSource>;
    lockedWeaponId?: string;
}

type WeaponSlotCategory = "Rifle" | "DMR/Sniper" | "LMG" | "SMG" | "Shotgun" | "Launcher" | "Handgun";

interface ArmoryItem {
    id: string;
    name: string;
    mod: ModSource;
    category: WeaponSlotCategory;
    caliber: Caliber;
    defaultMag: [string, number];
    opticType: OpticProfile;
    hasBipod: boolean;
...
const VALID_FACTIONS: readonly FactionName[] = ["NATO", "CSAT", "AAF", "FIA"] as const;
const VALID_ROLES: readonly RoleName[] = ["Rifleman", "Medic", "Marksman", "Anti-Tank", "Machine Gunner", "Sniper", "Pilot", "Pointman"] as const;
const VALID_MODS: readonly ModSource[] = ["Vanilla", "RHS", "CUP", "NIArms", "Custom"] as const;
const VALID_CALIBERS: readonly Caliber[] = [
    "5.56x45", "5.45x39", "7.62x39", "7.62x51", "7.62x54",
    "6.5x39", "5.8x42", "9x21", ".45ACP", "4.6x30", ".300WM",
    ".338", "9.3x64", ".408", "12.7x108", ".50BMG", "12Gauge", "Rocket"
] as const;
const VALID_OPTIC_PROFILES: readonly OpticProfile[] = ["cqb", "mid", "long"] as const;

function isValidCaliber(cal: string): cal is Caliber {
    return (VALID_CALIBERS as readonly string[]).includes(cal);
}

function isValidOpticProfile(opt: string): opt is OpticProfile {
    return (VALID_OPTIC_PROFILES as readonly string[]).includes(opt);
}

function isValidFaction(faction: string): faction is FactionName {
    return (VALID_FACTIONS as readonly string[]).includes(faction);
}
...
const WEAPONS_PART1: readonly WeaponDef[] = [
...
const LAUNCHERS: Record<FactionName, readonly LauncherDef[]> = {
    NATO: [
...
const OPTICS_POOL: Record<OpticProfile, readonly OpticDef[]> = {
    cqb: [
...
const MUZZLES_BY_CALIBER: Record<string, readonly MuzzleDef[]> = {
    "5.56x45": [
...
const FACTION_GEAR: Record<FactionName, FactionGear> = {
    NATO: {
        ...
        handgunMag: ["16Rnd_9x21_Mag", 16] as [string, number],
    ...
    AAF: {
        ...
        handgunMag: ["9Rnd_45ACP_Mag", 9] as [string, number],
...
const COMMON_ITEMS = Object.freeze({
    ...
    nvg: { NATO: "NVGoggles", CSAT: "NVGoggles_OPFOR", AAF: "NVGoggles_INDEP", FIA: "NVGoggles" } as Record<FactionName, string>,
...
const FALLBACK_WEAPON: WeaponDef = BUILTIN_WEAPONS[0];
...
class RandomUtils {
     ...
     */
    public static choice<T>(arr: readonly T[] | T[], fallbackValue?: T): T {
        if (!arr || arr.length === 0) {
...
class WeaponRepository {
    private static readonly STORAGE_KEY = "arma3_custom_weapons";
    private static customWeaponsCache: WeaponDef[] | null = null;

    // Fast-lookup index caches
    private static allWeaponsCache: WeaponDef[] | null = null;
    private static weaponsByModCache: Map<ModSource, WeaponDef[]> | null = null;
     ...
     */
    public static getCustomWeapons(forceRefresh: boolean = false): WeaponDef[] {
     ...
     */
    public static saveCustomWeapons(weapons: WeaponDef[]): boolean {
        try {
     ...
     */
    public static getAllWeapons(): WeaponDef[] {
     ...
     */
    public static getWeaponsByMod(): Map<ModSource, WeaponDef[]> {
        if (this.weaponsByModCache !== null) {
            return this.weaponsByModCache;
        }

        const map = new Map<ModSource, WeaponDef[]>();
     ...
     */
    public static isValidWeaponDef(obj: unknown): obj is WeaponDef {
        if (!obj || typeof obj !== "object") return false;
        const candidate = obj as Partial<WeaponDef>;

        if (typeof candidate.id !== "string" || candidate.id.trim() === "") return false;
        if (!Array.isArray(candidate.factions) || candidate.factions.length === 0) return false;
        if (!Array.isArray(candidate.roles) || candidate.roles.length === 0) return false;
        if (!Array.isArray(candidate.defaultMag) || candidate.defaultMag.length < 2) return false;
        if (typeof candidate.defaultMag[0] !== "string" || candidate.defaultMag[0].trim() === "") return false;
        if (typeof candidate.defaultMag[1] !== "number" || candidate.defaultMag[1] <= 0) return false;
     ...
     */
    public static sanitizeWeapon(input: Partial<WeaponDef>): WeaponDef {
     ...
     */
    public static getById(id: string): WeaponDef | undefined {
...
class LoadoutEngine {
    private static lastWeaponId: string | null = null;

    /**
     * Primary loadout generator. Operates without any direct DOM dependencies.
     */
    public static generate(options: GeneratorOptions): { loadoutData: LoadoutData; sqf: string } {
        const factionsList = VALID_FACTIONS;
        const rolesList: RoleName[] = ["Rifleman", "Medic", "Marksman", "Anti-Tank", "Machine Gunner", "Sniper"];

        let faction: FactionName;
        let role: RoleName;
        ...
        let lockedLauncherDef: LauncherDef | undefined;
        let lockedHandgunClass: string | undefined;
        ...
        const activeMods = options.activeMods.size > 0
            ? options.activeMods
            : new Set<ModSource>(["Vanilla"]);
        ...
        if (lockedPrimary) {
            ...
            const candidateWeapons: WeaponDef[] = [];
            ...
            const availablePool = candidateWeapons.length > 0
                ? candidateWeapons
                : WeaponRepository.getAllWeapons();

            // 2. Filter weapons based on Chaos Level
            let roleFiltered: WeaponDef[] = [];
            ...
            if (this.lastWeaponId && roleFiltered.length > 1) {
                const withoutLast = roleFiltered.filter(w => w.id !== this.lastWeaponId);
        ...
        let opticProfile: OpticProfile = weapon.opticType;
        if (role === "Sniper") {
            opticProfile = "long";
        ...
        if (chaos === 3 && RandomUtils.bool(0.4) && !lockedPrimary) {
        ...
        const hasOptic = lockedPrimary ? true : RandomUtils.bool(0.55);
        const optic = (hasOptic && validOptics.length > 0) ? RandomUtils.choice(validOptics).id : "";

        // 4. Caliber-Accurate Muzzle Suppressor
        let muzzle = "";
        const suppressorChance = lockedPrimary ? 0.7 : (chaos === 1 ? 0.2 : (chaos === 2 ? 0.75 : 0.5));
        ...
        const handgunMag: [string, number] = [...factionGear.handgunMag];
        ...
        const cGear = FACTION_GEAR[clothingFaction] || FACTION_GEAR.NATO;
        ...
        const nvg = RandomUtils.bool(nvgChance) ? (COMMON_ITEMS.nvg[faction] || "NVGoggles") : "";
...
interface MetaPreset {
    ...
    faction: FactionName;
    role: RoleName;
    mod: ModSource | "Hybrid";
    title: string;
    description: string;
    primaryClass: string;
    primaryMag: [string, number];
    optic: string;
    muzzle: string;
    pointer: string;
    bipod: string;
    ...
    launcherMag?: [string, number];
    handgunClass?: string;
    handgunMag?: [string, number];
    handgunMuzzle?: string;
    uniform?: string;
    vest?: string;
    backpack?: string;
    headgear?: string;
...
const META_PRESETS: readonly MetaPreset[] = [
    // =========================================================================
...
class BestLoadoutFactory {
     ...
     */
    public static create(faction: FactionName, role: RoleName, modSuite: ModSource | "Hybrid" = "Hybrid"): { loadoutData: LoadoutData; sqf: string } {
        ...
        const handgunMag: [string, number] = match.handgunMag || [...gear.handgunMag];
     ...
     */
    public static getAllPresets(filter?: { faction?: string; role?: string; mod?: string }): MetaPreset[] {
     ...
     */
    public static rollMeta(
        factionInput: FactionName | "Random" = "Random",
        roleInput: RoleName | "Random" = "Random",
        activeMods?: Set<ModSource>
    ): { loadoutData: LoadoutData; sqf: string; presetTitle: string } {
        const faction = (factionInput === "Random" || !isValidFaction(factionInput))
            ? RandomUtils.choice(VALID_FACTIONS, "NATO")
            : factionInput;

        const roleCandidates: RoleName[] = ["Pointman", "Rifleman", "Medic", "Marksman", "Machine Gunner", "Sniper", "Anti-Tank"];
...
class SqfSerializer {
     ...
     */
    public static serialize(data: LoadoutData): string {
     ...
     */
    public static formatPretty(sqf: string): string {
...
function generateLoadoutWithOptions(options: GeneratorOptions): { loadoutData: LoadoutData; sqf: string } {
    return LoadoutEngine.generate(options);
}

function generateBestLoadout(faction: FactionName, role: RoleName): { loadoutData: LoadoutData; sqf: string } {
    return BestLoadoutFactory.create(faction, role);
}

function generateSQF(data: LoadoutData): string {
...
function generateLoadout(
    factionInput: string = "NATO",
    roleInput: string = "Rifleman"
): { loadoutData: LoadoutData; sqf: string } {
    let chaos = 1;
    const activeMods = new Set<ModSource>();
    ...
    if (typeof document !== "undefined" && typeof document.getElementById === "function") {
        ...
        const checkMod = (id: string, mod: ModSource) => {
            const el = document.getElementById(id) as HTMLInputElement | null;
            if (el && el.checked) activeMods.add(mod);
        };
    ...
    const faction = (factionInput === "Random" || isValidFaction(factionInput))
        ? (factionInput as FactionName | "Random")
        : "Random";

    const role = (roleInput === "Random" || isValidRole(roleInput))
        ? (roleInput as RoleName | "Random")
        : "Random";
...
// ----------------------------------------------------------------------------
// 9. Armory & Catalog Controller
// ----------------------------------------------------------------------------
// 9. Armory Asset & Weapon Preview Resolvers
// ----------------------------------------------------------------------------

interface PlatformFirearmInfo {
    platform: string;
    manufacturer: string;
    origin: string;
    imageFile: string;
...
const REAL_FIREARM_PLATFORMS: Record<string, PlatformFirearmInfo> = {
...
class WeaponPhotoResolver {
    public static getPlatformFamily(weaponId: string, weaponName: string = "", category: string = ""): string {
    ...
    public static isSvgOrVector(url: string): boolean {
    ...
    public static getCaliberFallbackPhoto(caliber: string): string {
        switch (caliber) {
    ...
    public static sanitizePhotoUrl(url: string, caliber: string = "5.56x45", category: string = "Rifle"): string {
    ...
    public static getRealWeaponPhotoUrl(weapon: { id: string; name?: string; category?: string; caliber?: string; photoUrl?: string }): string {
    ...
    public static getCategoryFallbackPhoto(category: string): string {
        switch (category) {
    ...
    public static getWeaponMetadata(weaponId: string, weaponName: string = "", category: string = ""): { platform: string; manufacturer: string; origin: string } {
...
class WeaponAssetResolver {
    ...
    public static getCategoryPhoto(category: WeaponSlotCategory): string {
        return WeaponPhotoResolver.getCategoryFallbackPhoto(category);
    }

    public static getCaliberPhoto(caliber: Caliber): string {
    ...
    public static getCategorySvg(category: WeaponSlotCategory | string): string {
        return WeaponPhotoResolver.getCategoryFallbackPhoto(category);
    }

    public static getPrimaryImageUrl(item: ArmoryItem | { id: string }): string {
        return `assets/weapons/photos/${item.id}.png`;
    }
}

// ----------------------------------------------------------------------------
// 10. Armory & Weapon Catalog Controller
// ----------------------------------------------------------------------------

class ArmoryController {
    private static catalog: ArmoryItem[] = [];
    private static activeCategory: string = "all";
    private static activeMod: string = "all";
    private static searchQuery: string = "";
    private static isInitialized: boolean = false;
    private static currentlyInspectedWeapon: ArmoryItem | null = null;
    private static activeTabId: string = "generator-view";
    private static isScrollBound: boolean = false;

    private static elements: {
        grid: HTMLElement | null;
        searchInput: HTMLInputElement | null;
        searchClearBtn: HTMLElement | null;
        countBadge: HTMLElement | null;
        categoryPills: HTMLElement | null;
        modPills: HTMLElement | null;
        emptyState: HTMLElement | null;
        resetFiltersBtn: HTMLElement | null;
        inspectModal: HTMLElement | null;
        modalWeaponName: HTMLElement | null;
        modalWeaponMod: HTMLElement | null;
        modalWeaponCategory: HTMLElement | null;
        modalWeaponCaliber: HTMLElement | null;
        modalWeaponId: HTMLElement | null;
        modalCopyIdBtn: HTMLElement | null;
        modalCopyFeedback: HTMLElement | null;
        modalDefaultMag: HTMLElement | null;
        modalOpticProfile: HTMLElement | null;
        modalSlotsInfo: HTMLElement | null;
        modalOpticsCount: HTMLElement | null;
        modalOpticsList: HTMLElement | null;
        modalMuzzlesCount: HTMLElement | null;
        modalMuzzlesList: HTMLElement | null;
        modalFactionsList: HTMLElement | null;
        modalRolesList: HTMLElement | null;
        modalCloseBtn: HTMLElement | null;
        modalCancelBtn: HTMLElement | null;
        modalRollBtn: HTMLElement | null;
        modalWeaponImg?: HTMLElement | null;
        modalPreviewModWatermark?: HTMLElement | null;
        modalWeaponManufacturer?: HTMLElement | null;
        modalWeaponOrigin?: HTMLElement | null;
        modalWeaponOriginPill?: HTMLElement | null;
    ...
    public static init(): void {
        ...
        this.elements = {
            grid: getEl<HTMLElement>("armory-grid"),
            searchInput: getEl<HTMLInputElement>("armory-search-input"),
            searchClearBtn: getEl<HTMLElement>("armory-search-clear"),
            countBadge: getEl<HTMLElement>("armory-count-badge"),
            categoryPills: getEl<HTMLElement>("category-filter-pills"),
            modPills: getEl<HTMLElement>("mod-filter-pills"),
            emptyState: getEl<HTMLElement>("armory-empty-state"),
            resetFiltersBtn: getEl<HTMLElement>("armory-reset-filters-btn"),
            inspectModal: getEl<HTMLElement>("armory-inspect-modal"),
            modalWeaponName: getEl<HTMLElement>("modal-weapon-name"),
            modalWeaponMod: getEl<HTMLElement>("modal-weapon-mod"),
            modalWeaponCategory: getEl<HTMLElement>("modal-weapon-category"),
            modalWeaponCaliber: getEl<HTMLElement>("modal-weapon-caliber"),
            modalWeaponId: getEl<HTMLElement>("modal-weapon-id"),
            modalCopyIdBtn: getEl<HTMLElement>("modal-copy-id-btn"),
            modalCopyFeedback: getEl<HTMLElement>("modal-copy-feedback"),
            modalDefaultMag: getEl<HTMLElement>("modal-default-mag"),
            modalOpticProfile: getEl<HTMLElement>("modal-optic-profile"),
            modalSlotsInfo: getEl<HTMLElement>("modal-slots-info"),
            modalOpticsCount: getEl<HTMLElement>("modal-optics-count"),
            modalOpticsList: getEl<HTMLElement>("modal-optics-list"),
            modalMuzzlesCount: getEl<HTMLElement>("modal-muzzles-count"),
            modalMuzzlesList: getEl<HTMLElement>("modal-muzzles-list"),
            modalFactionsList: getEl<HTMLElement>("modal-factions-list"),
            modalRolesList: getEl<HTMLElement>("modal-roles-list"),
            modalCloseBtn: getEl<HTMLElement>("modal-close-btn"),
            modalCancelBtn: getEl<HTMLElement>("modal-cancel-btn"),
            modalRollBtn: getEl<HTMLElement>("modal-roll-btn"),
            modalWeaponImg: getEl<HTMLImageElement>("modal-weapon-img"),
            modalPreviewModWatermark: getEl<HTMLElement>("modal-preview-mod-watermark"),
            modalWeaponManufacturer: getEl<HTMLElement>("modal-weapon-manufacturer"),
            modalWeaponOrigin: getEl<HTMLElement>("modal-weapon-origin"),
            modalWeaponOriginPill: getEl<HTMLElement>("modal-weapon-origin-pill"),
            scrollTopBtn: getEl<HTMLElement>("armory-scroll-top-btn")
        };
    ...
    public static setActiveTab(tabId: string): void {
    ...
    public static buildCatalog(): ArmoryItem[] {
        ...
        for (const w of primaries) {
            ...
            let category: WeaponSlotCategory;
        ...
        const launcherDefs: Record<string, { name: string; mod: ModSource }> = {
        ...
        const handgunDefs: Record<string, { name: string; mod: ModSource; caliber: Caliber; defaultMag: [string, number]; hasMuzzle: boolean }> = {
    ...
    public static getCatalog(): ArmoryItem[] {
    ...
    public static filterCatalog(): ArmoryItem[] {
    ...
    private static bindEvents(): void {
        ...
        if (categoryPills) {
            categoryPills.addEventListener("click", (e) => {
                const target = (e.target as HTMLElement).closest(".filter-pill");
                if (!target) return;
        ...
        if (grid) {
            grid.addEventListener("click", (e) => {
                const card = (e.target as HTMLElement).closest(".weapon-card");
                if (!card) return;
    ...
    public static renderCatalog(): void {
    ...
    public static openInspectModal(weaponId: string): void {
        ...
        if (modalWeaponImg) {
            const imgEl = modalWeaponImg as HTMLImageElement;
            imgEl.onerror = () => {
                imgEl.onerror = null;
                imgEl.src = `${fallbackPhoto}?v=3`;
            };
            imgEl.src = `${photoUrl}?v=3`;
            imgEl.alt = `${item.name} Real Photo`;
        }
    ...
    public static closeInspectModal(): void {
    ...
    public static rollAroundWeapon(weaponId: string): void {
        const item = this.catalog.find(it => it.id === weaponId);
        if (!item) return;
...
class UIController {
    // Cached DOM elements
    private static elements: {
        tabBtns: NodeListOf<HTMLElement> | null;
        chaosSlider: HTMLInputElement | null;
        chaosLabel: HTMLElement | null;
        chaosHint: HTMLElement | null;
        generateBtn: HTMLButtonElement | null;
        copyBtn: HTMLButtonElement | null;
        loadoutGrid: HTMLDivElement | null;
        sqfOutput: HTMLElement | null;
        factionSelect: HTMLSelectElement | null;
        roleSelect: HTMLSelectElement | null;
        metaBadge: HTMLElement | null;
        bestContainer: HTMLElement | null;
        customTbody: HTMLElement | null;
        customCount: HTMLElement | null;
        addWeaponForm: HTMLFormElement | null;
        jsonArea: HTMLTextAreaElement | null;
        importJsonBtn: HTMLElement | null;
        exportJsonBtn: HTMLElement | null;
        resetCustomBtn: HTMLElement | null;
        modVanilla: HTMLInputElement | null;
        modRhs: HTMLInputElement | null;
        modCup: HTMLInputElement | null;
        modNiarms: HTMLInputElement | null;
        modCustom: HTMLInputElement | null;
        metaFactionPills: NodeListOf<HTMLButtonElement> | null;
        metaRolePills: NodeListOf<HTMLButtonElement> | null;
        metaModPills: NodeListOf<HTMLButtonElement> | null;
        metaCountBadge: HTMLElement | null;
        rollMetaBtn: HTMLButtonElement | null;
    } | null = null;

    private static currentSQF: string = "";
    private static isBestLoadoutsRendered: boolean = false;
    private static activeMetaFaction: string = "all";
    private static activeMetaRole: string = "all";
    private static activeMetaMod: string = "all";
    private static isMetaEventsBound: boolean = false;
    ...
    private static cacheElements(): void {
        ...
        this.elements = {
            ...
            chaosSlider: getEl<HTMLInputElement>("chaos-slider"),
            chaosLabel: getEl<HTMLElement>("chaos-label"),
            chaosHint: getEl<HTMLElement>("chaos-hint"),
            generateBtn: getEl<HTMLButtonElement>("generate-btn"),
            copyBtn: getEl<HTMLButtonElement>("copy-btn"),
            loadoutGrid: getEl<HTMLDivElement>("loadout-grid"),
            sqfOutput: getEl<HTMLElement>("sqf-output"),
            factionSelect: getEl<HTMLSelectElement>("faction-select"),
            roleSelect: getEl<HTMLSelectElement>("role-select"),
            metaBadge: getEl<HTMLElement>("loadout-meta-badge"),
            bestContainer: getEl<HTMLElement>("best-loadouts-container"),
            customTbody: getEl<HTMLElement>("custom-weapons-tbody"),
            customCount: getEl<HTMLElement>("custom-count"),
            addWeaponForm: getEl<HTMLFormElement>("add-weapon-form"),
            jsonArea: getEl<HTMLTextAreaElement>("custom-json-area"),
            importJsonBtn: getEl<HTMLElement>("import-json-btn"),
            exportJsonBtn: getEl<HTMLElement>("export-json-btn"),
            resetCustomBtn: getEl<HTMLElement>("reset-custom-mods-btn"),
            modVanilla: getEl<HTMLInputElement>("mod-vanilla"),
            modRhs: getEl<HTMLInputElement>("mod-rhs"),
            modCup: getEl<HTMLInputElement>("mod-cup"),
            modNiarms: getEl<HTMLInputElement>("mod-niarms"),
            modCustom: getEl<HTMLInputElement>("mod-custom"),
            metaFactionPills: document.querySelectorAll<HTMLButtonElement>("#meta-faction-pills .pill"),
            metaRolePills: document.querySelectorAll<HTMLButtonElement>("#meta-role-pills .pill"),
            metaModPills: document.querySelectorAll<HTMLButtonElement>("#meta-mod-pills .pill"),
            metaCountBadge: getEl<HTMLElement>("meta-count-badge"),
            rollMetaBtn: getEl<HTMLButtonElement>("roll-meta-btn")
        };
    }

    private static bindTabNavigation(): void {
    ...
    public static switchToTab(targetId: string): void {
        const { tabBtns } = this.elements || {};
        if (tabBtns) {
            tabBtns.forEach(b => {
        ...
        const views = ["generator-view", "armory-view", "best-loadouts-view", "mod-manager-view"];
        views.forEach(id => {
        ...
        // Lazy render Best Loadouts view on first tab activation
    ...
    public static runGenerateWithWeapon(weaponId: string, preferredFaction?: FactionName, preferredRole?: RoleName, requiredMod?: ModSource): void {
        ...
        const faction = (factionSelect?.value || "Random") as FactionName | "Random";
        const role = (roleSelect?.value || "Random") as RoleName | "Random";
    ...
    private static bindChaosSlider(): void {
        const { chaosSlider, chaosLabel, chaosHint } = this.elements!;
        if (!chaosSlider || !chaosLabel || !chaosHint) return;

        const updateChaosUI = (val: number) => {
    ...
    private static bindGeneratorControls(): void {
    ...
    private static getActiveMods(): Set<ModSource> {
        const mods = new Set<ModSource>();
        const { modVanilla, modRhs, modCup, modNiarms, modCustom } = this.elements!;

        if (modVanilla?.checked) mods.add("Vanilla");
        if (modRhs?.checked) mods.add("RHS");
        if (modCup?.checked) mods.add("CUP");
        if (modNiarms?.checked) mods.add("NIArms");
        if (modCustom?.checked) mods.add("Custom");

        if (mods.size === 0) {
            mods.add("Vanilla");
            if (modVanilla) modVanilla.checked = true;
        }

        return mods;
    }

    private static runGenerate(): void {
        const {
            chaosSlider,
            factionSelect,
            roleSelect,
    ...
    private static renderCard(label: string, name: string, attachments: string[] = []): string {
        if (!name) return "";
    ...
    private static bindMetaFilterEvents(): void {
        ...
        if (rollMetaBtn) {
            rollMetaBtn.addEventListener("click", () => {
                const faction = this.activeMetaFaction === "all" ? "Random" : (this.activeMetaFaction as FactionName);
                const role = this.activeMetaRole === "all" ? "Random" : (this.activeMetaRole as RoleName);
                const activeMods = this.getActiveMods();
     ...
     */
    private static renderBestLoadouts(): void {
        ...
        if (!this.isMetaEventsBound) {
            bestContainer.addEventListener("click", (e) => {
                const copyBtn = (e.target as HTMLElement).closest(".copy-best-btn") as HTMLButtonElement | null;
                if (copyBtn) {
                ...
                const inspectBtn = (e.target as HTMLElement).closest(".inspect-best-btn") as HTMLButtonElement | null;
                if (inspectBtn) {
    ...
    private static bindCustomWeaponsManager(): void {
        const {
            addWeaponForm,
            importJsonBtn,
            exportJsonBtn,
        ...
        if (customTbody) {
            customTbody.addEventListener("click", (e) => {
                const deleteBtn = (e.target as HTMLElement).closest(".delete-custom-btn") as HTMLButtonElement | null;
                if (!deleteBtn) return;
    ...
    private static handleAddWeaponSubmit(): void {
        ...
        const caliber = getVal("custom-caliber") as Caliber;
        const modName = (getVal("custom-mod-name") || "Custom") as ModSource;
        const role = getVal("custom-role") as RoleName;
        const faction = getVal("custom-faction") as FactionName;
        const opticType = getVal("custom-optic-type") as OpticProfile;
        const hasBipod = (document.getElementById("custom-has-bipod") as HTMLInputElement)?.checked || false;
        const hasMuzzle = (document.getElementById("custom-has-muzzle") as HTMLInputElement)?.checked || false;
        ...
        if (this.elements?.addWeaponForm) {
            this.elements.addWeaponForm.reset();
            const modInput = document.getElementById("custom-mod-name") as HTMLInputElement | null;
            if (modInput) modInput.value = "Custom";
        }
    ...
    private static renderCustomWeaponsTable(): void {
    ...
    private static async copyToClipboard(text: string, triggerBtn: HTMLElement, successText: string): Promise<void> {
        try {
            if (navigator?.clipboard?.writeText) {
...
// ----------------------------------------------------------------------------
// 11. Bootstrap & Environment Export Bridge
// ----------------------------------------------------------------------------
